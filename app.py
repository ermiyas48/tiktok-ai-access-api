...
def discovery():
    return {"service":"TikTok AI Access API","version":"1.1.0","base_url":BASE_URL or None,"oauth":"/auth/tiktok","docs":"/api/docs","openapi":"/openapi.json","capabilities":"/api/capabilities","endpoints":["/api/me","/api/profile","/api/stats","/api/account","/api/videos","/api/videos/query","/api/videos/{id}","/api/creator","/api/post/status/{publish_id}","/api/video/upload","/api/video/publish","/api/data/request","/api/data","/api/data/status","/api/data/download","/api/export","/api/export/{request_id}"],"safety":{"destructive_actions":False,"delete_content":False,"delete_account":False,"cancel_data_requests":False,"note":"No TikTok content/account deletion endpoint exists. Data-request cancellation is intentionally disabled."},"boundary":"TikTok exposes public video metadata through Display API and broader account categories through approved Data Portability exports; no fake live comments, DMs, or follower-list endpoints are created."}
...
def capabilities(request:Request):
    ...
    for label,s in [("Profile","user.info.basic"),("Profile details","user.info.profile"),("Statistics","user.info.stats"),("Videos","video.list"),("Publishing","video.publish"),("Draft upload","video.upload"),("Data export","portability.all.single"),("Data export ongoing","portability.all.ongoing"),("Posts/profile export","portability.postsandprofile.single"),("Posts/profile export ongoing","portability.postsandprofile.ongoing"),("Activity export","portability.activity.single"),("Activity export ongoing","portability.activity.ongoing"),("Messages export","portability.directmessages.single"),("Messages export ongoing","portability.directmessages.ongoing")]:items[label]={"authorized":s in scopes,"scope":s}
    return {"connected":True,"open_id":c.open_id,"authorized_scopes":sorted(scopes),"capabilities":items,"safety":{"destructive_actions":False,"delete_content":False,"delete_account":False,"cancel_data_requests":False}}
...
LEGAL_STYLE = """body{font-family:system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;max-width:850px;margin:40px auto;padding:20px;line-height:1.6;color:#222}a{color:#0645ad}h1,h2{line-height:1.25}footer{margin-top:40px;padding-top:20px;border-top:1px solid #ddd;font-size:.9em}"""

@app.get("/terms", response_class=HTMLResponse)
def terms():
    return HTMLResponse("""<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Terms of Service</title><style>""" + LEGAL_STYLE + """</style></head><body><h1>Terms of Service</h1><p><strong>Effective date:</strong> September 16, 2026</p><p>This service, TikTok AI Access API, provides an interface for authorized access to TikTok account data and features through TikTok's official authorization mechanisms.</p><h2>Use of the service</h2><p>You may use the service only for lawful purposes and only with TikTok accounts and content you are authorized to access or manage. You are responsible for complying with TikTok's terms, policies, and applicable law.</p><h2>TikTok authorization</h2><p>Account access is granted through TikTok OAuth. The service does not request or store your TikTok password. You may revoke access through TikTok or by using the account controls provided by the service.</p><h2>Content and responsibility</h2><p>You retain responsibility for content and actions performed through your connected TikTok account. The service does not claim ownership of your TikTok content.</p><h2>Availability</h2><p>The service is provided on an as-available basis. Features, endpoints, and availability may change as the service evolves or as TikTok changes its APIs or policies.</p><h2>Contact</h2><p>For service questions or requests, use the contact information provided by the service operator.</p><footer><a href="/privacy">Privacy Policy</a> &nbsp;|&nbsp; <a href="/">Home</a></footer></body></html>""")

@app.get("/privacy", response_class=HTMLResponse)
def privacy():
    return HTMLResponse("""<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Privacy Policy</title><style>""" + LEGAL_STYLE + """</style></head><body><h1>Privacy Policy</h1><p><strong>Effective date:</strong> September 16, 2026</p><p>This policy explains how the TikTok AI Access API handles information received when you authorize your TikTok account.</p><h2>Information received</h2><p>When you authorize the service, it may receive the TikTok account information and API data permitted by the scopes you approve. OAuth access and refresh tokens are stored by the service to maintain the authorized connection.</p><h2>How information is used</h2><p>Information is used to provide the API functions you request, maintain your authorized connection, and process API requests to TikTok on your behalf.</p><h2>Storage</h2><p>Connection data is stored by the service on its hosting infrastructure. The service does not request or store your TikTok password.</p><h2>Sharing</h2><p>The service sends requests to TikTok's APIs as necessary to provide the authorized functionality. It does not sell connected-account data.</p><h2>Revocation and deletion</h2><p>You can revoke the application's TikTok authorization through TikTok. You may also request deletion of stored connection data from the service operator.</p><h2>Changes</h2><p>This policy may be updated when the service or its data handling changes. The effective date above identifies the current version.</p><footer><a href="/terms">Terms of Service</a> &nbsp;|&nbsp; <a href="/">Home</a></footer></body></html>""")

@app.get("/")
def home():
    return HTMLResponse(f"<html><body style='font-family:system-ui;max-width:850px;margin:50px auto;padding:20px'><h1>TikTok AI Access API</h1><div style='padding:20px;border:1px solid #ddd;border-radius:12px'><h2>Connect TikTok</h2><p>TikTok Login Kit OAuth 2.0. No password is requested here.</p><a href='/auth/tiktok'>Connect TikTok</a></div><p>AI API Base URL: <code>{BASE_URL or '(set BASE_URL)'}</code></p><p><a href='/api'>API</a> · <a href='/api/docs'>Docs</a> · <a href='/openapi.json'>OpenAPI</a> · <a href='/terms'>Terms</a> · <a href='/privacy'>Privacy</a></p></body></html>")
@app.get("/connected")
def connected():
    with Session() as db:c=db.query(Connection).order_by(Connection.id.desc()).first()
...
def videos(request:Request,cursor:Optional[int]=None,limit:int=20):
    auth_guard(request);c=conn();scope(c,"video.list");body={"max_count":max(1,min(limit,20)),"fields":["id","create_time","cover_image_url","share_url","video_description","duration","height","width","title","embed_html","embed_link","like_count","comment_count","share_count","view_count","is_aigc"]};body.update({"cursor":cursor} if cursor is not None else {});return tiktok(c,"POST","/v2/video/list/",json=body).json()
@app.get("/api/videos/{video_id}")
def video(video_id:str,request:Request):auth_guard(request);c=conn();scope(c,"video.list");return tiktok(c,"POST","/v2/video/query/",json={"filters":{"video_ids":[video_id]},"fields":["id","create_time","cover_image_url","share_url","video_description","duration","height","width","title","embed_html","embed_link","like_count","comment_count","share_count","view_count","is_aigc"]}).json()
@app.post("/api/videos/query")
def videos_query(body:VideoQueryIn,request:Request):
    auth_guard(request);c=conn();scope(c,"video.list")
    return tiktok(c,"POST","/v2/video/query/",json={"filters":{"video_ids":body.video_ids},"fields":["id","create_time","cover_image_url","share_url","video_description","duration","height","width","title","embed_html","embed_link","like_count","comment_count","share_count","view_count","is_aigc"]}).json()

@app.get("/api/account")
def account(request:Request):
    auth_guard(request);c=conn()
    scopes=set((c.scopes or "").split())
    out={"connected":True,"open_id":c.open_id,"authorized_scopes":sorted(scopes)}
    if "user.info.basic" in scopes: out["basic"]=tiktok(c,"GET","/v2/user/info/",params={"fields":"open_id,union_id,avatar_url,display_name"}).json()
    if "user.info.profile" in scopes: out["profile"]=tiktok(c,"GET","/v2/user/info/",params={"fields":"open_id,display_name,avatar_url,profile_deep_link,bio_description,is_verified,username"}).json()
    if "user.info.stats" in scopes: out["stats"]=tiktok(c,"GET","/v2/user/info/",params={"fields":"follower_count,following_count,likes_count,video_count"}).json()
    out["safety"]={"destructive_actions":False,"delete_content":False,"delete_account":False,"cancel_data_requests":False}
    return out
@app.get("/api/creator")
